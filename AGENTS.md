...
**`pnpm e2e:live` is that second half** (`scripts/e2e-live.mjs` + `scripts/live/NN-*.mjs`, one file per section: keys, roles, tax, invoices, lifecycle, seats/top-ups/usage, mid-cycle documents, plan moves, roles/isolation, workspace close). Test keys only — refused otherwise — every object prefixed `live<ts>` and torn down LIFO in a `finally`. `E2E_ENV_FILE` picks the environment, `E2E_ONLY=02,05` a subset. **Never in CI**, because it creates real objects in whichever account the keys name.
