...
**Dunning / past_due** is reflected via the polled `customer.subscription.updated` event (→ `adapter.setSubscription(status)`), and Stripe **Smart Retries** + the card-updater handle recovery (Dashboard config, no code). A **Customer Portal** is the no-code option for self-serve plan management.
