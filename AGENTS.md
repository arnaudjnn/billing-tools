...
- `WorkOSOrgAdapter`'s metadata writers send DELETIONS AS EXPLICIT NULLS: WorkOS's metadata update *merges* (measured, orgs and users alike — a key omitted from the patch is KEPT), so the old read-merge-locally-and-omit shape never deleted anything, and every clear this adapter issued (pendingPlan after a resume, an emptied member store) silently no-oped. The SDK types metadata as `Record<string, string>`; the cast to send null is deliberate.
- `pollWorkOSEvents` walks pages by hand: `events.listEvents` returns a plain `List`, not an `AutoPaginatable`.
- **Event polling instead of webhooks** (`events.ts`): a product choice — zero webhook secret, zero dashboard. **ONE EVENT MUST NOT WEDGE THE SWEEP**, and it did: a handler throw left the stored cursor untouched so the page would be retried, which is right for a transient failure and fatal for a permanent one — and permanent is the common case, because a Stripe account shared across environments emits events carrying an `org_id` this deployment does not have. Measured: a consumer's cursor sat on one such event for **twelve days**, and since state mirroring is deliberately the poller's job and not the webhook's, nothing mirrored for ANY org in that time — no `past_due`, no plan change, no cancellation — with a log line as the only signal. Both legs now retry an event a few times inside the sweep, then REPORT it (`onEventFault`) and advance past it. Skipping costs a delayed mirror for one org and mirroring is idempotent and self-healing; wedging costs every org, indefinitely. A LIST failure still throws — an API that cannot be reached is the sweep failing, not one event, and swallowing it would baseline the cursor over unread history. The two legs also run **independently**: a Stripe-side problem used to stop the WorkOS mirror behind it.
- `invitations.accept` TRIES `createOrganizationMembership` + `revokeInvitation` first, and falls back to WorkOS's own `acceptInvitation`. The create leads because it is the only path that serves a `canAccept` hook widening acceptance to a verified secondary email — there the accepting user is not the invited one, and WorkOS's accept would enrol the wrong account. **It is a fallback, not a replacement, and the earlier claim that WorkOS's accept "needs the invited user's session" was simply false** — it takes an invitation id against the API key. That wrong reason is what kept the fallback from being written: SENDING an invitation already creates a **pending** membership, so the create is refused with a bare `GenericServerException` carrying `code: "cannot_reactivate_pending_organization_membership"`, only `ConflictException` was tolerated, and **acceptance therefore threw for every ordinary invitee**. Invisible because every test and fixture adds members straight through WorkOS; found the first time a browser walked the link. Match the CODE — there is no class to narrow to, and a `status === 400` guard would swallow every other bad request.
...
- **What was BOUGHT is recorded by the change itself, not by the event.** `changePlan` wrote plan/status/subscriptionId/periodEnd and nothing about seats, so a workspace that added one **paid for it and did not receive it** — `seatCounts` is the member ceiling, the seat-assignment capacity AND the size of a `cap.perSeat` pool, so the customer was charged and stayed refused at all three. Measured: Stripe's item read quantity 4 while the org still said 3. Waiting for `customer.subscription.updated` is not good enough, for the same reason `completeCheckout` mirrors on return — the customer is looking at the page that says what they just bought, and a deployment on event POLLING (or on neither) may not see it for minutes or at all. It writes the BREAKDOWN only, read off the UPDATED subscription, from the same `purchasedSeatsOf` the event handler uses; whichever lands second is a no-op rather than a second opinion. A `pending` change records nothing, because nothing was granted.
...
**The member ceiling is what was BOUGHT, not `limits.members`.** On a plan that sells seats, `memberSeats` binds on the seats the subscription actually pays for and falls back to `limits.members` only where that cannot be read. Reading the plan constant alone was a hole with a price on it: `limits.members: 100` against 3 purchased seats admitted 100 people, and under `cap: per_seat` every one of them drew a full pack — **measured on a live workspace at 3 seats sold, 10 members, `aggregate.limit` 10 000 against 3 000 paid for**. Nothing refused and nothing reported it, because the two numbers had never been compared. `limitSource` (`"purchased"` | `"plan"`) travels with the number, because the remedies are opposite: every bought seat taken is fixed by buying another, and a plan that stops at N people is not — telling that owner to buy a seat sends them to a form that refuses them. Unreadable counts as UNKNOWN and falls back to the plan cap, the same fail-open `seatAssignable` makes: refusing every invitation during a WorkOS blip is worse than the giveaway.
...
**`ensureAccountTaxId` had never worked, in three ways, and only a real account could say so.**
It sent `owner: { type: "account" }`, which Stripe refuses ("Must provide `account` if you
provide `type=account`"); your own account id there is refused too ("No such account"), because
`account` means a CONNECTED account — the shape for yourself is **`owner: { type: "self" }`**.
And making it the invoice DEFAULT cannot be done over the API at all: `accounts.update` on your
own account answers "you may only use it on connected accounts", so that half is a **Dashboard
setting** (Settings → Invoicing → Default tax IDs). The attempt stays, because a platform doing
this TO a connected account genuinely can; the refusal is reported as `isDefault: false` rather
than thrown, since the id itself was created and that is the part no Dashboard visit can do for
you, and the doctor names the manual step. Meanwhile `account_tax_ids` on an invoice is how a
caller puts the number on a document today without touching the Dashboard. It had **no test at
all** — a fake accepts any params — which is why `scripts/live/14-mounted-seams.mjs` found it
and `tests/account-tax-id.test.mjs` now reproduces both refusals.
...
**`canManage` reaches the CURRENT card too.** Its CTA returned before the check, so a viewer who may not manage the plan saw a grid of dead buttons and one live one — on the card about the plan they cannot change, which reads as the button they ARE allowed to press. `kind` stays `current` (the card still means "you are here"); `disabledReason` answers the separate question of what the viewer may DO with it, and a consumer disables on the reason alone.

**A pricing grid shown to a customer is a grid of moves UP.** `derivePlanViews({ upgradesOnly: true })` drops the tiers below `currentPlan` — a card headed "Get the Hobby plan" offered to somebody on Pro is a downgrade dressed as an upgrade, and cancelling belongs on the billing page, in those words, scheduled for the period end. Ranked by `planRank`, the same arithmetic `isDowngrade` uses, so the grid cannot offer a move the engine treats as going the other way. Inert without a `currentPlan` (and for an unknown one), which is what lets ONE page serve a signed-out reader and a signed-in customer rather than two components that drift.
