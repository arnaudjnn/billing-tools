...
- **Pattern A — WorkOS-only** (shipped `WorkOSOrgAdapter`, used by **gtm-tools**): `orgId` *is* the WorkOS org id. Orgs, org API keys and `stripeCustomerId` live in WorkOS; no other storage. Import from `@arnaudjnn/billing-tools/adapters/workos-org`.
...
**`runBillingDoctor` is the same command for an app that has plans.** `checkPlansConfig` needs the app's own catalogue, so the bin subcommand cannot run it and each app keeps a ~12-line script carrying only what that app knows: its `plans`, its `config`, and what its ledger `covers`. The `--url` / `--no-webhook` parsing, the run order (config first — it needs no network and explains most account-level symptoms) and the exit arithmetic live here.
