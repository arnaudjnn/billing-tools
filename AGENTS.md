...
**`src/tax.ts` is the default path, and it is not Stripe Tax.** `resolveTax` works the rate out locally from `eu-vat-rates-data` (45 European countries, tracked daily from the European Commission's own TEDB) with VIES for B2B validation, and `taxRatesFor` applies the answer as an explicit Stripe **TaxRate** on the line items — no per-transaction fee, and no Stripe Tax *registration objects* needed to calculate (distinct from `config.tax.registrations`, below, which is a statement about your own obligations and changes the answer). `updateCheckoutSessionTaxRates` re-taxes an open session when the typed country differs from the one guessed, which is the piece Stripe Tax would otherwise do. What you take on instead: evidence-of-location records for EU B2C, threshold monitoring, and filing. The safe direction is already chosen — a tax number VIES cannot verify falls back to CHARGING tax, because wrongly charging is recoverable and wrongly exempting means owing it yourself.
...
**A European seller exporting outside the EU is OUT OF SCOPE, not unknown.** `TaxDecision` distinguishes the two, and conflating them was a bug worth naming: every non-European destination was marked `approximate` regardless of origin, which would have refused every export a French business made. The place of supply for a digital service is the customer's country; if that is outside the EU, no EU VAT arises and 0% is a complete answer (`outOfScope: true`). `approximate` is reserved for the case where tax IS due and this library has no rate for where it is due — then 0% would be a guess, and it is refused. A separate obligation can still arise in the destination once a nexus threshold is crossed, but that is a registration question — and `registrations`, below, is how you answer it.

**A rate is charged where the SELLER's regime says tax is due — never because the dataset happens to have a number for the destination.** The dataset covers 45 European countries and only 27 of them are in the EU, and reading coverage as liability was a money bug: **GB, CH, NO, TR and IS** passed `isKnownCountry`, fell through every EU branch and were charged their own domestic rate, so an Italian seller invoiced **20% "VAT" to a UK customer** and 25% to a Norwegian one. Neither is collectable — not EU VAT (place of supply is outside the EU, so none arises) and not UK or Norwegian VAT without a registration there — so it took a fifth of the invoice with nowhere to pay it over, which is the fault `oss: false` exists to prevent, one border further out. Those destinations are now `outOfScope`, and a domestic sale inside them (a UK seller billing a UK customer) is untouched.

**Reverse charge does NOT require the seller to be in the EU**, and requiring it billed a German business 19% MwSt on an invoice from a US supplier — tax that supplier cannot collect or remit. The customer self-accounts under Art. 44/196 whoever the supplier is. Without a valid id the sale is B2C and destination VAT is due under non-Union OSS with no threshold to sit under, which is why that one case is the exception to `registrations` below.

**But the VAT id must belong to the country the customer is IN, and that check was missing.** Reverse charge is for a taxable person established in *another* member state and the number is the evidence of WHERE, so a German address presenting an Italian id is a contradiction rather than a German business. Unchecked it was self-serve VAT avoidance on any B2B checkout: an Italian company typing a German address beside its own real Italian number was zero-rated by a seller who owed 22%. A mismatch is refused, not resolved — either field could be the wrong one, and charging is the recoverable direction.

**Greece needs its own format check, because the dataset's is unusable.** `eu-vat-rates-data` keys Greece as `GR` but writes its pattern as `^EL\d{9}$` — the only entry including the prefix every other entry omits (`DE` is plain `^\d{9}$`) — so `validateFormat` rejects *every* spelling of a Greek number (measured at 2026.7.1: `EL…`, `GR…` and `GREL…` all false). That silently withdrew reverse charge from one member state, charging every Greek business VAT it should never have paid, with a customer complaint as the only signal. `parseVatNumber` accepts either spelling, reports the country as `GR` (what an address carries) and canonicalises the id to `EL…` (what VIES routes on — it has no member state `GR`).

**`config.tax.registrations` is the second input a rate needs, and no dataset can supply it.** `origin` says where you are established; this says where you took on an obligation, which is what decides whether a sale is taxed at all. `[{ country: "IT" }, { country: "GB" }]` for a VAT registration, `[{ country: "US", state: "CA" }]` for US nexus — a country-wide entry covers every state, a state-scoped one only its own, and an address carrying no state matches no state-scoped entry (too vague to place inside a registration is not evidence that it falls inside one). **Undefined is "the caller did not say"**, never "registered nowhere" — the regime rules alone then decide, which is what every pre-existing deployment gets. Declared, there is ONE rule for everywhere including domestic, so `[]` says something omitting it cannot: a US-established seller with no nexus charges 0% on every sale, **correctly and without a refusal**. This is what makes a US destination answerable rather than a throw — post-Wayfair you must not collect in a state until you have nexus there — and it is why `state`, accepted and threaded from the customer's Stripe address since the beginning and never once read, is finally used.

**`allowApproximate` is GONE, and the refusal now has no override at all.** It was a flag whose only remaining use was to under-collect silently. Since the move off `sales-tax` there has been no non-European rate here to approximate WITH — an `approximate` decision carries `percent: 0`, so the flag never applied a state-level figure, it only decided whether 0% went out quietly; the error message claimed otherwise for as long as the deleted dependency had been gone. And once `registrations` exists, both of its cases collapse: where you are **not** registered, `registrations` already answers 0% *completely* and needs no permission; where you **are**, suppressing the throw invoices 0% on tax you owe, which is the one unrecoverable direction this whole file is written against. Measured while deciding it: `approximate` is **unreachable** for a European-origin seller that declares no registrations, so the flag was already dead config for both consumers.

The escape hatches are now the two that assert something — `registrations` (you do not owe it there) and `mode: "stripe"` (you do, so have Stripe compute it) — plus `mode: "none"` to charge nothing at all, deliberately and deployment-wide. **Migration:** delete `allowApproximate` from `config.tax`; if a deployment was relying on it, `registrations: []` reproduces the 0% while stating why, and `mode: "none"` reproduces it for an account that charges no tax anywhere.

**`oss` decides ONE case, and it is the case a B2B seller actually hits.** Reverse charge needs a VALID VAT id, so a cross-border EU customer without one is not reverse-chargeable and has to be taxed somewhere: OSS-registered (`oss` defaults to true) at the CUSTOMER's rate, not registered (`oss: false`) at YOUR OWN — which is what the sub-€10 000 regime allows and the only rate you can remit without a foreign registration. Charging the customer's rate while unregistered collects VAT with nowhere to pay it over. It fires more often than it sounds: a customer below its own registration threshold, a typo, or VIES unreachable, where this library charges rather than exempts. Nothing else moves — domestic stays domestic, a valid id still reverse charges, non-EU is still out of scope — and the display name follows the rate, or the invoice says MwSt above an Italian figure.

**The VIES lookup is injectable (`__setVatValidatorForTests`), because the suite is offline by design.** The reverse-charge tests asserted on a real German company's live registration status, so they broke that contract and went red whenever a member state's VIES node returned `MS_UNAVAILABLE` — the exact outage the charge-rather-than-exempt fallback exists for, and the one behaviour that could not be tested while the test *was* the outage. The local format check is deliberately outside the seam: a stub that also accepted malformed numbers would test a laxer function than ships.
...
**Its tax arrives as a THUNK, and that is not a style choice.** `tryAutoReload` takes `ChargeTax | (() => Promise<ChargeTax>)` and calls the thunk only past every early return — below threshold, no card, nothing owed — because it is fired on **every wallet-funded metered call** and almost none of them reload. Resolving tax first meant a live **VIES** request plus a Stripe customer retrieve per metered call under `mode: "local"`. The failure that invites is a cascade, not an error: rate-limited at VIES, no number verifies, and an unverifiable number means CHARGE — so every B2B customer silently stops reverse-charging and the first signal is "why is everyone suddenly paying VAT". If you add a read to this function, add it after the early returns. *(Still open: VIES results are not cached, so a customer who reloads repeatedly re-validates each time. Caching POSITIVES is safe — a valid registration stays valid; caching negatives is not, because it would extend an outage's over-charging past the outage.)*
