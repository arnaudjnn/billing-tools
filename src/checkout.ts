...
//
// ONE path: `createCheckoutSession`, a Checkout Session in elements or hosted mode.
//
// There used to be a second — `createSubscription`/`updateSubscription`/`cancelSubscription`,
// a `default_incomplete` subscription the browser confirmed — and it was DELETED rather than
// kept, because the reason it existed had become the reason not to. A subscription created
// that way must know its total before the customer types anything, so its tax can only be a
// rate picked in advance: one country, no reverse charge, wrong the moment somebody buys from
// another member state. Nothing in the library called it, neither consumer used it, and its
// only driver was a `useCheckout` hook nobody mounted — so what it actually offered was a
// second charge path with none of the guards the real one has (no basket validation, no
// schedule release, no idempotency key). A duplicated charge path is not safe because the
// numbers currently match.
//
...
//
// The reason is tax, and it is why the older `default_incomplete` path is gone rather
// than optional. Such a subscription must know its total BEFORE the customer types
// anything, so tax can only be a rate you picked
