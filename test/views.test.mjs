...
test('plan config derives the pricing surfaces', async () => {
  ...
  const PLANS = definePlans({
    hobby: { sells: { kind: "nothing" }, cap: { kind: "pool", tokens: 1000, onExhausted: "block" },
    ...
    pro: { sells: { kind: "seats", minSeats: 2, maxSeats: 100, seatTypes: {
        standard: { price: { monthly: 2104, yearly: 21600 }, includedTokens: 1000, min: 1, display: { label: "Posto Standard", usage: "Tutte le funzioni" } },
        premium: { price: { monthly: 10523, yearly: 108000 }, includedTokens: 5000, display: { label: "Posto Premium", usage: "5× l'utilizzo" } },
        api: { price: { monthly: 52615, yearly: 540000 }, includedTokens: 25000, shared: true, max: 1, display: { label: "Posto API" } } } },
    ...
    enterprise: { sells: { kind: "flat", price: { monthly: 0, yearly: 500000 }, intervals: ["yearly"] },
      grant: { kind: "none" }, cap: { kind: "pool", tokens: 1000000, onExhausted: "block" },
  ...
  ok("hobby is free with a pool allowance", hobby.price.kind === "free" && hobby.included.scope === "pool" && hobby.included.tokens === 1000);
  ...
  ok("pro included = pack for the default basket", pro.included.scope === "per_seat" && pro.included.tokens === 2000,
     `${pro.included.tokens} (minSeats 2 × 1000)`);
  ...
  ok("enterprise package size visible at last", ent.included.tokens === 1000000 && ent.included.scope === "pool");
  ...
  const GTM = definePlans({
    hobby: { sells: { kind: "flat", price: { monthly: 1000, yearly: 10000 } }, grant: { kind: "none" },
      cap: { kind: "pool", tokens: 1000, onExhausted: "wallet" }, limits: { members: 1 }, sale: "self_serve",
      display: { name: "Hobby", order: 1 } },
    pro: { sells: { kind: "flat", price: { monthly: 5000, yearly: 50000 } }, grant: { kind: "none" },
      cap: { kind: "pool", tokens: 5000, onExhausted: "wallet" }, limits: { members: 10 }, sale: "self_serve",
