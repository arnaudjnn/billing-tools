...
test("a sold seat type reports its badge, falling back to the label", () => {
  // BOTH forms, and `label` collapses them while `badge` does not. That distinction is the
  // point: a pill wants the short word, and a row that already says "posto" wants it too —
  // but a consumer with only the collapsed value was stripping the noun off "Posto Premium"
  // with a regex, in two files, to get back what the config had said in the first place.
